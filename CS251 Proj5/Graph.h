...
private:
	...
	struct vertex {
		...
		vertex(int _id = 0, string _name = "", int _numInputs = 0, int _numOutputs = 0)
			: id{ _id }, name{ _name }, numInputs{ _numInputs }, numOutputs{ _numOutputs }
		{ }
	};
	...
	bool bfs(int src, std::vector<vertex_label> & report) {
		...
		while (!q.empty()) {
			...
			for (edge& e : vertices[u].outgoing) {
				v = e.vertex_id;
				travelDistance++;
				report[v].npaths = travelDistance;
...
public:
	 ...
	 */
	 ...
	 // recursive helper function doing most of the work
	void _ep(const vector<vertex_label>& rpt, int dest, vector<int>path) {
		int u;
		if (rpt[dest].dist == 0) { // must be the source
			path.push_back(dest);
			return;
		}
		// find a viable predecessor!
		for (edge& e : vertices[dest].incoming) {
			u = e.vertex_id; // testing edge (u, dest)
			if (rpt[u].dist + 1 == rpt[dest].dist) { // Aha!
				_ep(rpt, u, path);
				path.push_back(dest); // important to do this after
				// recursive call
				return;
			}
		}
		// should never reach here (rpt would be bad if we did)
	}
	...
	bool extract_path(const vector<vertex_label> & rpt, int dest, vector<int> & path) {
		...
		_ep(rpt, dest, path);
		// your code here!
		//if (rpt[dest].dist == -1 || rpt[dest].pred == -1) {
		//	//path.insert(path.begin(), dest); //experimental
		//	return false;
		//}

		//while (rpt[dest].dist != 0) {
	...
	bool dag_critical_paths(vector<vertex_label> & rpt) {
		...
		topo_sort(sortedVector);

		//initialize
		int numNodes = num_nodes();
		vertex_label tempLabel;
		tempLabel.dist = -1;
		tempLabel.pred = -1;

		for (int i = 0; i < numNodes; i++) {
			rpt.push_back(tempLabel);
		}

		string sortedString = id_vec2string(sortedVector); //for debugging purposes only
		//go through each vertex
		for (int i = 0; i < sortedVector.size(); i++) {
			
			//find the source which is when sources are 0
			if ((rpt[sortedVector[i]].dist == -1) || (rpt[sortedVector[i]].pred == -1)) {
				rpt[sortedVector[i]].dist = 0;
				rpt[sortedVector[i]].pred = sortedVector[i];
			}

			for (edge& e : vertices[i].outgoing) {
				int id = e.vertex_id;

				if (rpt[id].dist < rpt[sortedVector[i]].dist + e.weight) {
					rpt[id].dist = rpt[sortedVector[i]].dist + e.weight;
					rpt[id].pred = sortedVector[i];
				}
			}
		}

		//source are the 0 incoming
		return true;
	}
	...
	bool dag_num_paths(vector<vertex_label> & rpt) {
		...
		for (int i = 0; i < sortedVector.size(); i++) {
			...
			//if source vertex
			if (vertices[sortedVector[i]].incoming.size() == 0) {
				vertices[sortedVector[i]].numInputs = 1;
			}
			else {
				//for each incoming
				for (edge e : vertices[sortedVector[i]].incoming) {
					int id = e.vertex_id;
					vertices[sortedVector[i]].numInputs += vertices[id].numInputs;
				}
			}

		}

		//go backwards on second pass
		//basically reverse of the first pass
		vector<int> revSortedVector;
		topo_sort(revSortedVector);
		
		std::reverse(revSortedVector.begin(), revSortedVector.end());

		string revSortedString = id_vec2string(revSortedVector); //for debugging purposes only

		for (int i = 0; i < revSortedVector.size(); i++) {
			//if sink vertex
			if (vertices[revSortedVector[i]].outgoing.size() == 0) {
				vertices[revSortedVector[i]].numOutputs = 1;
			}
			else {
				//for each outgoing
				for (edge e : vertices[revSortedVector[i]].outgoing) {
					int id = e.vertex_id;
					vertices[revSortedVector[i]].numOutputs += vertices[id].numOutputs;
					int x = 0;
				}
			}
		}
		...
		for (int i = 0; i < sortedVector.size(); i++) {
			rpt[sortedVector[i]].npaths = (vertices[sortedVector[i]].numInputs) * (vertices[sortedVector[i]].numOutputs);
		}

		//queue like structure
		//keep track of the 0 outgoing or the source nodes
		//then how many path to the target node (d)
		//it has 3 incoming
		//from d to j has 4 differnt paths
		//which is 4
		//and 3 * 4 = 12 

		//first pass
		//when going down look at incoming of u and except for source they have one
		//so d has value 3 because it has 3 incoming values

		//second pass
		//going up mutiplies by outgoing the way up
		//start with one with bottom node
		//the outgoing of u adopts the number then adds them and then mutiply up of this second pass and first pass
		//adding in ques and when done remove from queue
	...
	bool valid_topo_order(const vector<int> & order) {
		...
		for (int i = 0; i < order.size(); i++) {
			...
			for (edge e : vertices[order[i]].incoming) {
				...
				//now if any of the vertex is not process then it is not a valid topological sorting
				if (visited[id] == false) {
					return false;
				}
			}
	...
	bool enum_paths(int target, vector<string> & paths) {
		...
		// your code here!
		string pathAsString;
		_enum_paths(target, pathAsString, paths);
		return true;
	}

	void _enum_paths(int target, string pathAsString, vector<string>& paths) {
		if (pathAsString.empty() == true) {
			pathAsString.append(vertices[target].name);
		}
		else {
			pathAsString.insert(0, vertices[target].name + " ");
		}

		if (vertices[target].incoming.size() == 0) {
			paths.push_back(pathAsString);
			return;
		}

		for (edge e : vertices[target].incoming) {
			int id = e.vertex_id;
			_enum_paths(id, pathAsString, paths);
		}
	}
